...
let basePath=RouterElement.baseUrlSansHost(),shortUrl=url.substr(basePath.length);RouterElement._route={url:shortUrl// Check if all current routes wil let us navigate away
       ...
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */RouterElement._activeRouters[0].router.dispatchEvent(new CustomEvent("onRouteCancelled",{bubbles:!0,composed:!0,detail:{shortUrl}}));return}RouterElement._activeRouters=[];this.prepareRoutersForDispatch();if(0===RouterElement._routers.length){this._currentMatch={remainder:shortUrl,data:null,redirect:null,url:"",useCache:!1};this.hasMatch=!1}else if((await RouterElement.performMatchOnRouters(shortUrl,RouterElement._routers))&&!0!==skipHistory){RouterElement.updateHistory(url);RouterElement.updateAnchorsStatus()}}/** Updates the location history with the new href */static updateHistory(href){let urlState=RouterElement.getUrlState(),url=urlState;if(2===url.length){url=href}else{url=document.baseURI+url}// Need to use a full URL in case the containing page has a base URI.

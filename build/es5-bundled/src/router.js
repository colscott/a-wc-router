...
basePath=RouterElement.baseUrlSansHost();shortUrl=url.substr(basePath.length);RouterElement._route={url:shortUrl// Check if all current routes wil let us navigate away
       ...
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */RouterElement._activeRouters[0].router.dispatchEvent(new CustomEvent("onRouteCancelled",{bubbles:!0,composed:!0,detail:{shortUrl:shortUrl}}));return _context10.abrupt("return");case 6:RouterElement._activeRouters=[];this.prepareRoutersForDispatch();_context10.next=10;return RouterElement.performMatchOnRouters(shortUrl,RouterElement._routers);case 10:_context10.t0=_context10.sent;if(!_context10.t0){_context10.next=13;break}_context10.t0=!0!==skipHistory;case 13:if(!_context10.t0){_context10.next=16;break}RouterElement.updateHistory(url);RouterElement.updateAnchorsStatus();case 16:case"end":return _context10.stop();}}},_callee10,this)}));function dispatch(_x13,_x14){return _dispatch.apply(this,arguments)}return dispatch}()/** Updates the location history with the new href */},{key:"updateHistory",value:function updateHistory(href){var urlState=RouterElement.getUrlState(),url=urlState;if(2===url.length){url=href}else{url=document.baseURI+url}// Need to use a full URL in case the containing page has a base URI.
var fullNewUrl=new URL(url,window.location.protocol+"//"+window.location.host).href,oldRoute=RouterElement._route,now=window.performance.now(),shouldReplace=oldRoute._lastChangedAt+RouterElement._dwellTime>now;oldRoute._lastChangedAt=now;if(shouldReplace){window.history.replaceState({},"",fullNewUrl)}else{window.history.pushState({},"",fullNewUrl)}}/**
     ...
     */},{key:"updateAnchorsStatus",value:function(){var _updateAnchorsStatus=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee11(url,links){var currentAnchors,nextAnchors,i,iLen,link,urlFragments,j,jLen,urlFragment,urlFragNamedItemMatch,_i,_iLen,_link,named,routes,k,kLen,_k,_kLen,routeUrl;return regeneratorRuntime.wrap(function _callee11$(_context11){while(1){switch(_context11.prev=_context11.next){case 0:url=url||RouterElement.getUrlState();console.log("updateAnchorsStatus",url);currentAnchors=links||RouterElement._anchors;nextAnchors=[];// Tidy up any unconnected anchors
for(i=0,iLen=currentAnchors.length;i<iLen;i++){if(!0===currentAnchors[i].a.isConnected){link=currentAnchors[i];nextAnchors[nextAnchors.length]=link;link.a.classList.remove(link.a.activeClassName||"active")}}urlFragments=url.split("::");j=0,jLen=urlFragments.length;case 7:if(!(j<jLen)){_context11.next=61;break}urlFragment=urlFragments[j];_context11.next=11;return NamedRouting.parseNamedItem(urlFragment,!0);case 11:urlFragNamedItemMatch=_context11.sent;_i=0,_iLen=nextAnchors.length;case 13:if(!(_i<_iLen)){_context11.next=58;break}_link=nextAnchors[_i];if(!(_link&&_link.a.classList.contains(_link.a.activeClassName||"active"))){_context11.next=17;break}return _context11.abrupt("continue",55);case 17:if(!_link){_context11.next=55;break}if(!_link.routerMatches){_context11.next=55;break}named=_link.routerMatches.named;routes=_link.routerMatches.routes;if(!urlFragNamedItemMatch){_context11.next=38;break}k=0,kLen=named.length;case 23:if(!(k<kLen)){_context11.next=38;break}if(!(named[k].name==urlFragNamedItemMatch.name)){_context11.next=35;break}if(!(named[k].url==urlFragNamedItemMatch.urlEscaped)){_context11.next=31;break}// full match on named item
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",55);case 31:if(!(0===urlFragNamedItemMatch.urlEscaped.indexOf(named[k].url))){_context11.next=35;break}// full match on named item
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",55);case 35:k++;_context11.next=23;break;case 38:urlFragment="/"===urlFragment[0]?urlFragment.substr(1):urlFragment;_k=0,_kLen=routes.length;case 40:if(!(_k<_kLen)){_context11.next=55;break}routeUrl="/"===routes[_k][0]?routes[_k].substr(1):routes[_k];if(!(urlFragment==routeUrl)){_context11.next=48;break}// full match on route
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",55);case 48:if(!(0===urlFragment.indexOf(routeUrl))){_context11.next=52;break}// partial match on route
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",55);case 52:_k++;_context11.next=40;break;case 55:_i++;_context11.next=13;break;case 58:j++;_context11.next=7;break;case 61:/**
       * Event that fires when HTMLAnchorElement active statuses are being updated as part of a routing.
       * @event RouterElement#onRouteCancelled
       * @type CustomEvent
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */window.dispatchEvent(new CustomEvent("onLinkActiveStatusUpdated",{bubbles:!0,composed:!0,detail:{links:nextAnchors}}));return _context11.abrupt("return",null);case 63:case"end":return _context11.stop();}}},_callee11)}));function updateAnchorsStatus(_x15,_x16){return _updateAnchorsStatus.apply(this,arguments)}return updateAnchorsStatus}()/** Gets the current URL state based on currently active routers and outlets. */},{key:"getUrlState",value:function getUrlState(routers){var url=NamedRouting.generateNamedItemsUrl();routers=routers||RouterElement._routers;if(routers){for(var i=0,iLen=routers.length;i<iLen;i++){var router=routers[i],nextFrag=router.generateUrlFragment();if(nextFrag){if(url.length){url+="::"}url+=nextFrag;var childRouters=router._routers;if(childRouters&&childRouters.length){if(1===childRouters.length){url+="/"+this.getUrlState(childRouters)}else{url+="/("+this.getUrlState(childRouters)+")"}}}}}return url}/**
...
RouterElement._anchors=nextAnchors}/**
     ...
     */ /**
         ...
         * 
         */},{key:"sanitizeLinkHref",value:function(){var _sanitizeLinkHref=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee14(hrefSource){var href,url,hash,path,query,basePathLength,urlStr,urls,namedMatches,routerMatches,i,iLen,namedMatch;return regeneratorRuntime.wrap(function _callee14$(_context14){while(1){switch(_context14.prev=_context14.next){case 0:console.info(hrefSource&&hrefSource);href=RouterElement._getSameOriginLinkHref(hrefSource);console.info(href);url=new URL(href);hash=RouterElement._getHash();path=decodeURIComponent(url.pathname);query=url.search.substring(1);basePathLength=RouterElement.baseUrlSansHost().length;urlStr=path.substr(basePathLength);if("("===urlStr[0]){urlStr=urlStr.substr(1,urlStr.length-2)}urls=RouterElement.splitUrlIntoRouters(urlStr);namedMatches=[];routerMatches=[];i=0,iLen=urls.length;case 14:if(!(i<iLen)){_context14.next=22;break}_context14.next=17;return NamedRouting.parseNamedItem(urls[i],!0);case 17:namedMatch=_context14.sent;if(namedMatch){namedMatches.push(namedMatch)}else{routerMatches.push(urls[i])}case 19:i++;_context14.next=14;break;case 22:return _context14.abrupt("return",{named:namedMatches,routes:routerMatches});case 23:case"end":return _context14.stop();}}},_callee14)}));function sanitizeLinkHref(_x21){return _sanitizeLinkHref.apply(this,arguments)}return sanitizeLinkHref}()}]);function RouterElement(){var _this;babelHelpers.classCallCheck(this,RouterElement);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(RouterElement).call(this));/**

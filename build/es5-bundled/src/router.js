...
var urlEscaped=match[2].replace(/_dot_/g,"."),routeCancelled=!1;if(!0!==supressAdding){if(!1===NamedRouting.addAssignment(match[1],urlEscaped)){routeCancelled=!0}}return{name:match[1],url:match[2],urlEscaped:urlEscaped,cancelled:routeCancelled,namedOutlet:NamedRouting.parseNamedOutletUrl(match[2])}}return null}/**
     ...
     */},{key:"prefetchNamedOutletImports",value:function prefetchNamedOutletImports(namedAssignment){if(namedAssignment.namedOutlet&&namedAssignment.namedOutlet.options&&namedAssignment.namedOutlet.options.import){NamedRouting.pageReady().then(function(){return NamedRouting.importCustomElement(namedAssignment.namedOutlet.options.import)})}}},{key:"importCustomElement",value:function importCustomElement(importSrc,tagName){if(importSrc&&customElements.get(tagName)===void 0){import(importSrc)}}/**
     * 
     ...
     */},{key:"canLeave",value:function canLeave(newRoute){/**
       ...
       */var canLeaveEvent=new CustomEvent("onRouteLeave",{bubbles:!0,cancelable:!0,composed:!0,detail:{route:newRoute}});this.dispatchEvent(canLeaveEvent);return!canLeaveEvent.defaultPrevented}}]);return NamedRouting}();NamedRouting.registry={};NamedRouting.assignments={};var namedRouting={NamedRouting:NamedRouting},RouterElement=/*#__PURE__*/function(_HTMLElement){babelHelpers.inherits(RouterElement,_HTMLElement);babelHelpers.createClass(RouterElement,[{key:"disconnectedCallback",value:function disconnectedCallback(){RouterElement.removeRouter.call(this._parentRouter,this);this.removeEventListener("routerAdded",this.addRouter,!1);if(this.getName()){NamedRouting.removeNamedItem(this.getName())}}},{key:"connectedCallback",value:function connectedCallback(){if(!this.created){this.created=!0;// IE workaround for the lack of document.baseURI property
var baseURI=document.baseURI;if(baseURI===void 0){var baseTags=document.getElementsByTagName("base");baseURI=baseTags.length?baseTags[0].href:document.URL;document.baseURI=baseURI}this._routers=[];RouterElement.initialize()}if(this.isConnected){/**
...
if(href===window.location.href){return}var url=new URL(href),newUrl=RouterElement._getUrl(url);RouterElement.dispatch(newUrl)}/**
     ...
     */},{key:"dispatch",value:function dispatch(url){var basePath=RouterElement.baseUrlSansHost(),shortUrl=url.substr(basePath.length);RouterElement._route={url:shortUrl// Check if all current routes wil let us navigate away
       ...
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */RouterElement._activeRouters[0].router.dispatchEvent(new CustomEvent("onRouteCancelled",{bubbles:!0,composed:!0,detail:{shortUrl:shortUrl}}));return}RouterElement._activeRouters=[];if(RouterElement.performMatchOnRouters(shortUrl,RouterElement._routers)){RouterElement.updateHistory(url)}}/** Updates the location history with the new href */},{key:"updateHistory",value:function updateHistory(href){var urlState=RouterElement.getUrlState(),url=urlState;if(2===url.length){url=href}else{url=document.baseURI+url}// Need to use a full URL in case the containing page has a base URI.
...
if(this._routers&&match.remainder){RouterElement.performMatchOnRouters(match.remainder,this._routers)}}else{var outletElement=this._getRouterElements("an-outlet")[0];routeElement.getContent(match.data).then(function(content){return outletElement.renderContent(content)})}this.postProcessMatch()}return match}},{key:"postProcessMatch",value:function postProcessMatch(){this.hasMatch=!0;if(this._parentRouter._currentMatch){var parentMatch=this._parentRouter._currentMatch,remainder=parentMatch.remainder;// TODO get remainder from parent but ony take this routers url from it
...
window.addEventListener("navigate",RouterElement.handlerNavigate.bind(RouterElement),!1);RouterElement.changeUrl(decodeURIComponent(location.pathname))}}},{key:"_getSameOriginLinkHref",value:function _getSameOriginLinkHref(hrefSource){var href=null,anchor=null;if(babelHelpers.instanceof(hrefSource,Event)){var event=hrefSource;// We only care about left-clicks.
if(0!==event.button){return null}// We don't want modified clicks, where the intent is to open the page
// in a new tab.
if(event.metaKey||event.ctrlKey){return null}for(var eventPath=event.path,i=0,element;i<eventPath.length;i++){element=eventPath[i];if("A"===element.tagName&&element.href){anchor=element;break}}// If there's no link there's nothing to do.
...
var urlSpaceRegExp=RouterElement._makeRegExp(RouterElement.whiteListRegEx);if(urlSpaceRegExp&&!urlSpaceRegExp.test(normalizedHref)){return""}// Need to use a full URL in case the containing page has a base URI.
var fullNormalizedHref=new URL(normalizedHref,window.location.href).href;return fullNormalizedHref}},{key:"_makeRegExp",value:function _makeRegExp(urlSpaceRegex){return RegExp(urlSpaceRegex)}// ---------- Action helpers ----------
...
NamedRouting.addNamedItem(this)}RouterElement.initialize()}}},{key:"disconnectedCallback",value:function disconnectedCallback(){if(this.getName()){NamedRouting.removeNamedItem(this.getName())}}}]);function OutletElement(){var _this3;babelHelpers.classCallCheck(this,OutletElement);_this3=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(OutletElement).call(this));_this3.canLeave=NamedRouting.canLeave.bind(babelHelpers.assertThisInitialized(_this3));return _this3}babelHelpers.createClass(OutletElement,[{key:"getName",value:function getName(){if(this.outletName===void 0){this.outletName=this.getAttribute("name")}return this.outletName}},{key:"_createPathSegments",value:function _createPathSegments(url){return url.replace(/(^\/+|\/+$)/g,"").split("/")}/**
     ...
     */},{key:"renderContent",value:function renderContent(content){this.innerHTML="";// console.log('outlet rendered: ' + this.outletName, content);
if("string"===typeof content){this.innerHTML=content}else{this.appendChild(content)}this.dispatchOuletUpdated()}/**
     ...
     */},{key:"processNamedUrl",value:function(){var _processNamedUrl=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee2(url,supressUrlGeneration){var details,options,data,element;return regeneratorRuntime.wrap(function _callee2$(_context2){while(1){switch(_context2.prev=_context2.next){case 0:details=NamedRouting.parseNamedOutletUrl(url);options=details.options||{import:null};data=details.data||new Map;if(!1===babelHelpers.instanceof(data,Map)){data=new Map(Object.entries(data||{}))}// If same tag name then just set the data
if(!(this.children&&this.children[0]&&this.children[0].tagName.toLowerCase()==details.elementTag)){_context2.next=8;break}RouteElement.setData(this.children[0],data||{});this.dispatchOuletUpdated();return _context2.abrupt("return",this.children[0]);case 8:_context2.next=10;return NamedRouting.importCustomElement(options.import,details.elementTag);case 10:element=document.createElement(details.elementTag);RouteElement.setData(element,data||{});this.renderContent(element);if(!supressUrlGeneration){RouterElement.updateHistory("")}return _context2.abrupt("return",element);case 15:case"end":return _context2.stop();}}},_callee2,this)}));function processNamedUrl(_x,_x2){return _processNamedUrl.apply(this,arguments)}return processNamedUrl}()},{key:"dispatchOuletUpdated",value:function dispatchOuletUpdated(){/**
...
window.customElements.define("an-outlet",OutletElement);///@ts-check
var RouterLinkElement=/*#__PURE__*/function(_HTMLAnchorElement){babelHelpers.inherits(RouterLinkElement,_HTMLAnchorElement);babelHelpers.createClass(RouterLinkElement,[{key:"connectedCallback",value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(RouterLinkElement.prototype),"connectedCallback",this)&&babelHelpers.get(babelHelpers.getPrototypeOf(RouterLinkElement.prototype),"connectedCallback",this).call(this);RouterElement.initialize();window.dispatchEvent(new CustomEvent("routerLinksAdded",{detail:{links:[this]}}))}}]);function RouterLinkElement(){babelHelpers.classCallCheck(this,RouterLinkElement);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(RouterLinkElement).call(this))}return RouterLinkElement}(babelHelpers.wrapNativeSuper(HTMLAnchorElement));window.customElements.define("router-link",RouterLinkElement,{extends:"a"});export{namedRouting as $namedRouting,routesRoute as $routesRoute,routesRouter as $routesRouter,NamedRouting,RouteElement,RouterElement};
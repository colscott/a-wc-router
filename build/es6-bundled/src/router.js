...
for(let i=0,iLen=links.length;i<iLen;i++){const link=links[i];if(link.href){let matches=await RouterElement.sanitizeLinkHref(link);if(matches){if(activeClassName&&!link.hasAttribute("activeclassname")){link.setAttribute("activeclassname",activeClassName)}newAnchors[newAnchors.length]={a:link,activeClassName:activeClassName,routerMatches:matches};for(let j=0,jLen=matches.named.length;j<jLen;j++){NamedRouting.prefetchNamedOutletImports(matches.named[j])}}}}RouterElement._anchors=RouterElement._anchors.concat(newAnchors);RouterElement.updateAnchorsStatus(void 0,newAnchors)}static removeDisconnectedAnchors(){const currentAnchors=RouterElement._anchors,nextAnchors=[];// Tidy up any unconnected anchors

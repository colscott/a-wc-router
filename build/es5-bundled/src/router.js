...
for(i=0,iLen=currentAnchors.length;i<iLen;i++){if(!0===currentAnchors[i].a.isConnected){link=currentAnchors[i];nextAnchors[nextAnchors.length]=link;link.a.classList.remove(link.a.activeClassName||"active")}}urlFragments=url.split("::");j=0,jLen=urlFragments.length;case 6:if(!(j<jLen)){_context11.next=60;break}urlFragment=urlFragments[j];_context11.next=10;return NamedRouting.parseNamedItem(urlFragment,!0);case 10:urlFragNamedItemMatch=_context11.sent;_i=0,_iLen=nextAnchors.length;case 12:if(!(_i<_iLen)){_context11.next=57;break}_link=nextAnchors[_i];if(!(_link&&_link.a.classList.contains(_link.a.activeClassName||"active"))){_context11.next=16;break}return _context11.abrupt("continue",54);case 16:if(!_link){_context11.next=54;break}if(!_link.routerMatches){_context11.next=54;break}named=_link.routerMatches.named;routes=_link.routerMatches.routes;if(!urlFragNamedItemMatch){_context11.next=37;break}k=0,kLen=named.length;case 22:if(!(k<kLen)){_context11.next=37;break}if(!(named[k].name==urlFragNamedItemMatch.name)){_context11.next=34;break}if(!(named[k].url==urlFragNamedItemMatch.urlEscaped)){_context11.next=30;break}// full match on named item
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",54);case 30:if(!(0===urlFragNamedItemMatch.urlEscaped.indexOf(named[k].url))){_context11.next=34;break}// full match on named item
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",54);case 34:k++;_context11.next=22;break;case 37:urlFragment="/"===urlFragment[0]?urlFragment.substr(1):urlFragment;_k=0,_kLen=routes.length;case 39:if(!(_k<_kLen)){_context11.next=54;break}routeUrl="/"===routes[_k][0]?routes[_k].substr(1):routes[_k];if(!(urlFragment==routeUrl)){_context11.next=47;break}// full match on route
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",54);case 47:if(!(0===urlFragment.indexOf(routeUrl))){_context11.next=51;break}// partial match on route
_link.a.classList.add(_link.a.activeClassName||"active");_link=null;return _context11.abrupt("continue",54);case 51:_k++;_context11.next=39;break;case 54:_i++;_context11.next=12;break;case 57:j++;_context11.next=6;break;case 60:/**
       * Event that fires when HTMLAnchorElement active statuses are being updated as part of a routing.
       * @event RouterElement#onRouteCancelled
       * @type CustomEvent
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */window.dispatchEvent(new CustomEvent("onLinkActiveStatusUpdated",{bubbles:!0,composed:!0,detail:{links:nextAnchors}}));return _context11.abrupt("return",null);case 62:case"end":return _context11.stop();}}},_callee11)}));function updateAnchorsStatus(_x15,_x16){return _updateAnchorsStatus.apply(this,arguments)}return updateAnchorsStatus}()/** Gets the current URL state based on currently active routers and outlets. */},{key:"getUrlState",value:function getUrlState(routers){var url=NamedRouting.generateNamedItemsUrl();routers=routers||RouterElement._routers;if(routers){for(var i=0,iLen=routers.length;i<iLen;i++){var router=routers[i],nextFrag=router.generateUrlFragment();if(nextFrag){if(url.length){url+="::"}url+=nextFrag;var childRouters=router._routers;if(childRouters&&childRouters.length){if(1===childRouters.length){url+="/"+this.getUrlState(childRouters)}else{url+="/("+this.getUrlState(childRouters)+")"}}}}}return url}/**

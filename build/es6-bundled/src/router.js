...
let basePath=RouterElement.baseUrlSansHost(),shortUrl=url.substr(basePath.length);RouterElement._route={url:shortUrl// Check if all current routes wil let us navigate away
       ...
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */RouterElement._activeRouters[0].router.dispatchEvent(new CustomEvent("onRouteCancelled",{bubbles:!0,composed:!0,detail:{shortUrl}}));return}RouterElement._activeRouters=[];this.prepareRoutersForDispatch();if(0===RouterElement._routers.length){this._currentMatch={remainder:shortUrl,data:null,redirect:null,url:"",useCache:!1};this.hasMatch=!1}else if((await RouterElement.performMatchOnRouters(shortUrl,RouterElement._routers))&&!0!==skipHistory){RouterElement.updateHistory(url);RouterElement.updateAnchorsStatus()}}/** Updates the location history with the new href */static updateHistory(href){let urlState=RouterElement.getUrlState(),url=urlState;if(2===url.length){url=href}else if("/"===url){url=document.baseURI}else{url=document.baseURI+url}// Need to use a full URL in case the containing page has a base URI.
let fullNewUrl=new URL(url,window.location.protocol+"//"+window.location.host).href,oldRoute=RouterElement._route,now=window.performance.now(),shouldReplace=oldRoute._lastChangedAt+RouterElement._dwellTime>now;oldRoute._lastChangedAt=now;if(shouldReplace){window.history.replaceState({},"",fullNewUrl)}else{window.history.pushState({},"",fullNewUrl)}}/**
     ...
     */static async updateAnchorsStatus(url,links){url=url||RouterElement.getUrlState();let currentAnchors=links||RouterElement._anchors,nextAnchors=[];// Tidy up any unconnected anchors
for(let i=0,iLen=currentAnchors.length;i<iLen;i++){if(!0===currentAnchors[i].a.isConnected){let link=currentAnchors[i];nextAnchors[nextAnchors.length]=link;link.a.classList.remove(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active")}}let urlFragments=url.split("::");nextUrlFragment:for(let j=0,jLen=urlFragments.length;j<jLen;j++){let urlFragment=urlFragments[j],urlFragNamedItemMatch=await NamedRouting.parseNamedItem(urlFragment,!0);nextLink:for(let i=0,iLen=nextAnchors.length,link;i<iLen;i++){link=nextAnchors[i];if(link&&link.a.classList.contains(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active")){continue nextLink}if(link){if(link.routerMatches){let named=link.routerMatches.named,routes=link.routerMatches.routes;if(urlFragNamedItemMatch){for(let k=0,kLen=named.length;k<kLen;k++){if(named[k].name==urlFragNamedItemMatch.name){// TODO strip import out of both before compare
if(named[k].url==urlFragNamedItemMatch.urlEscaped){// full match on named item
link.a.classList.add(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active");link=null;continue nextLink}else//Check if it's a mtch upto data portion of url
if(0===urlFragNamedItemMatch.urlEscaped.indexOf(named[k].url)){// full match on named item
link.a.classList.add(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active");link=null;continue nextLink}}}}urlFragment="/"===urlFragment[0]?urlFragment.substr(1):urlFragment;for(let k=0,kLen=routes.length,routeUrl;k<kLen;k++){routeUrl="/"===routes[k][0]?routes[k].substr(1):routes[k];if(urlFragment==routeUrl){// full match on route
link.a.classList.add(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active");link=null;continue nextLink}else if(0===urlFragment.indexOf(routeUrl)){// partial match on route
link.a.classList.add(link.a.getAttribute("activeclassname")||link.a.activeClassName||"active");link=null;continue nextLink}}}}}}/**
       * Event that fires when HTMLAnchorElement active statuses are being updated as part of a routing.
       * @event RouterElement#onRouteCancelled
       * @type CustomEvent
       * @property {Object} details - The event details
       * @property {RouteElement} details.url - The url that was trying to be matched.
       */window.dispatchEvent(new CustomEvent("onLinkActiveStatusUpdated",{bubbles:!0,composed:!0,detail:{links:nextAnchors}}));return null}/**
...
for(let i=0,iLen=links.length;i<iLen;i++){const link=links[i];if(link.href){let matches=await RouterElement.sanitizeLinkHref(link);if(matches){if(activeClassName&&!link.hasAttribute("activeclassname")){link.setAttribute("activeclassname",activeClassName)}newAnchors[newAnchors.length]={a:link,routerMatches:matches};for(let j=0,jLen=matches.named.length;j<jLen;j++){NamedRouting.prefetchNamedOutletImports(matches.named[j])}}}}RouterElement._anchors=RouterElement._anchors.concat(newAnchors);RouterElement.updateAnchorsStatus(void 0,newAnchors)}static removeDisconnectedAnchors(){const currentAnchors=RouterElement._anchors,nextAnchors=[];// Tidy up any unconnected anchors
...
const dispatchRegisterLink=function(link){window.dispatchEvent(new CustomEvent("routerLinksAdded",{detail:{links:[link]}}))};class RouterLinkElement extends HTMLAnchorElement{/** @inheritdoc */connectedCallback(){RouterElement.initialize();dispatchRegisterLink(this)}/** @inheritdoc */static get observedAttributes(){return["href"]}/**
     * @inheritdoc
     * Listens for href attribute changing. If it does then it re-regesters the link.
     */attributeChangedCallback(name,oldValue,newValue){if("href"===name){if(oldValue&&newValue){dispatchRegisterLink(this)}}}/** @inheritdoc */constructor(){super()}}window.customElements.define("router-link",RouterLinkElement,{extends:"a"});export{namedRouting as $namedRouting,routesOutlet as $routesOutlet,routesRoute as $routesRoute,routesRouter as $routesRouter,NamedRouting,OutletElement,RouteElement,RouterElement};